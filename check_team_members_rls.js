...
async function inspect() {
    console.log('🔍 Inspecting RLS Policies for team_members...');

    const { data, error } = await supabase.rpc('get_rls_json', { table_names: ['team_members'] });

    if (error) {
        console.error('❌ Error inspecting policies:', error);
    } else {
        console.log('✅ Policies Found (writing to rls_output_team.json)');
        fs.writeFileSync('rls_output_team.json', JSON.stringify(data, null, 2));
    }
}
