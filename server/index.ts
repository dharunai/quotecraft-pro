...
app.post('/api/workflows/trigger', async (req, res) => {
  try {
    ...
    console.log(`[API] Triggering workflow event: ${event}`);

    // Import dynamically to avoid top-level await issues if any
    const { triggerWorkflows } = await import('./src/services/WorkflowEngine.ts');

    // Run asynchronously (don't block response)
    triggerWorkflows(event, data).catch(err =>
