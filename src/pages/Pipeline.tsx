...
export default function Pipeline() {
  ...
  const getStageColor = (stage: Stage) => {
    switch (stage) {
      case 'qualified': return 'bg-info/10 border-info/30';
      case 'proposal': return 'bg-warning/10 border-warning/30';
      case 'negotiation': return 'bg-primary/10 border-primary/30';
      case 'won': return 'bg-success/10 border-success/30';
      case 'lost': return 'bg-destructive/10 border-destructive/30';
  ...
  const weightedPipelineValue = deals
    ...
    .reduce((sum, d) => sum + ((d.deal_value || 0) * d.probability / 100), 0);
        ...
        <div className="grid grid-cols-5 gap-4 min-h-[600px]">
          {STAGES.map((stage) => {
            ...
            return (
              <div
                key={stage.id}
                className={`rounded-lg border-2 ${getStageColor(stage.id)} p-3`}
