...
export function QuotationPreview({
  ...
  return <div className="quotation-preview bg-white text-gray-900 max-w-4xl mx-auto" id="quotation-print">
      ...
      borderColor: settings.theme_color
        ...
        <div>
          {settings.logo_url ? <img src={settings.logo_url} alt={settings.company_name} className="h-16 mb-4 object-contain" /> : <h1 className="text-2xl font-bold mb-4" style={{
        ...
        <div className="text-right">
          <h2 className="text-3xl font-bold mb-4 font-sans" style={{
          color: settings.theme_color
        }}>
            QUOTATION
          </h2>
          <div className="text-sm space-y-1">
            ...
            {quotation.valid_until && <p><span className="text-gray-500">Valid Until:</span> {format(new Date(quotation.valid_until), 'dd MMM yyyy')}</p>}
          </div>
        </div>
      </div>
      ...
      <div className="p-6 border-b border-gray-200">
        <h3 className="text-sm font-medium text-gray-500 mb-2 font-sans">BILL TO</h3>
        <p className="font-semibold text-lg font-sans">{lead.company_name}</p>
        <p className="text-gray-700 font-sans">{lead.contact_name}</p>
        {lead.email && <p className="text-gray-600 text-sm font-sans">{lead.email}</p>}
        {lead.phone && <p className="text-gray-600 text-sm font-sans">{lead.phone}</p>}
        {lead.address && <p className="text-gray-600 text-sm font-sans">{lead.address}</p>}
      </div>
      ...
      <div className="p-6">
        <table className="w-full">
          ...
          <tbody>
            {items.map((item, index) => <tr key={item.id} className="border-b border-gray-200">
                ...
                <td className="px-4 py-3 text-right">
                  {settings.currency}{item.unit_price.toLocaleString('en-IN', {
                minimumFractionDigits: 2
              })}
                </td>
                <td className="px-4 py-3 text-right font-medium">
                  {settings.currency}{item.line_total.toLocaleString('en-IN', {
                minimumFractionDigits: 2
              })}
                </td>
              </tr>)}
          ...
          <div className="w-72 space-y-2">
            <div className="flex justify-between py-2 border-b border-gray-200">
              ...
              <span className="font-medium">
                {settings.currency}{subtotal.toLocaleString('en-IN', {
                minimumFractionDigits: 2
              })}
              </span>
            </div>
      ...
      {/* Notes */}
      {quotation.notes && <div className="px-6 pb-6">
          <h3 className="text-sm font-medium text-gray-500 mb-2">NOTES</h3>
          <p className="text-gray-700 text-sm whitespace-pre-wrap">{quotation.notes}</p>
        </div>}

      {/* Terms */}
      {settings.terms && <div className="px-6 pb-6 border-t border-gray-200 pt-6">
          <h3 className="text-sm font-medium text-gray-500 mb-2 font-sans">TERMS & CONDITIONS</h3>
          <p className="text-gray-600 text-sm whitespace-pre-wrap">{settings.terms}</p>
        </div>}

      {/* Footer */}
      <div className="px-6 py-4 text-center text-sm text-gray-500 border-t border-gray-200" style={{
