import React, { memo } from 'react';
import { Handle, Position, NodeProps } from '@xyflow/react';
import { Bell } from 'lucide-react';

interface NodeData {
  label?: string;
  title?: string;
  [key: string]: unknown;
}

const NotificationNode = memo(({ data, selected }: NodeProps) => {
  const nodeData = data as NodeData;
  return (
    <div className={`
      px-4 py-3 rounded-lg shadow-md border-2 min-w-[180px]
      bg-white dark:bg-slate-800 border-yellow-200 dark:border-yellow-700
      ${selected ? 'ring-2 ring-yellow-300 ring-offset-2' : ''}
    `}>
      <Handle
        type="target"
        position={Position.Top}
        className="!bg-yellow-500 !w-3 !h-3 !border-2 !border-white"
      />
      <div className="flex items-center gap-2">
        <div className="p-1.5 bg-yellow-500 rounded text-white">
          <Bell className="h-4 w-4" />
        </div>
        <div>
          <div className="text-[10px] uppercase tracking-wider text-muted-foreground">Action</div>
          <div className="font-semibold text-sm">{String(nodeData.label || 'Notification')}</div>
        </div>
      </div>
      {nodeData.title && (
        <div className="mt-2 text-xs text-muted-foreground truncate max-w-[160px]">
          {String(nodeData.title)}
        </div>
      )}
      <Handle
        type="source"
        position={Position.Bottom}
        className="!bg-yellow-500 !w-3 !h-3 !border-2 !border-white"
      />
    </div>
  );
});

NotificationNode.displayName = 'NotificationNode';

export default NotificationNode;
